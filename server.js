		...
		.then(result => {
			if (result === 0) {
				...
				users.insertOne(newUser).then(() => {
					...
					if (!req.body.noIndex === "on" && !req.body.email.includes("noIndex") && !req.body.email.includes("89jv4hu8283e4c9h")) {
					...
					// Add random nouns
					if (req.body.nouns === "on" || req.body.email.includes("random@nouns") || req.body.email.includes("kmocowiamcwaiom")) {
